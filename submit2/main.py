...
def process(inpath, outpath):
    wv = KeyedVectors.load('./word_vector_0116.kv',mmap='r')
    sess=tf.Session()
    new_saver = tf.train.import_meta_graph('./checkpoint_dir/model20190116.model.meta')
    new_saver.restore(sess, tf.train.latest_checkpoint('./checkpoint_dir'))
    graphp = tf.get_default_graph()
    inputx1 = graphp.get_tensor_by_name("input_x1:0")
    inputx2 = graphp.get_tensor_by_name("input_x2:0")
    outputy = graphp.get_tensor_by_name("score/output_y:0")
