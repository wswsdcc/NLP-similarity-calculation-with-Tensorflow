...
trained_word2vec_model_path = "../../sgns.target.word-word.dynwin5.thr10.neg5.dim300.iter5"
wv_save_path = "../word_vector.kv"
...
# sim = word_vectors.most_similar(u'蛋白质',topn=5)
# print '\n蛋白质-top10:'
# for item in sim:
#     print item[0],item[1]

def model2WvKeyedVector(model_path,wv_path):
...
print "begin to make test data"
test_labels, test_sens1, test_sens2 = makeData(test_file_name, wv)
test_fea1, test_fea2 = align2MaxSenLen(test_sens1, test_sens2)
...
with tf.variable_scope('sentence1'):
    lstm_cell_1 = tf.nn.rnn_cell.LSTMCell(num_units=512) 
    lstm_cell_2 = tf.nn.rnn_cell.LSTMCell(num_units=256) 
    lstm_cell_3 = tf.nn.rnn_cell.LSTMCell(num_units=128)
...
with tf.variable_scope('sentence2'):
    lstm_cell_4 = tf.nn.rnn_cell.LSTMCell(num_units=512) 
    lstm_cell_5 = tf.nn.rnn_cell.LSTMCell(num_units=256) 
    lstm_cell_6 = tf.nn.rnn_cell.LSTMCell(num_units=128) 
...
with tf.name_scope("optimize"):
    losses = tf.losses.mean_squared_error(input_y,logits)
    optim = tf.train.GradientDescentOptimizer(0.5).minimize(losses)
...
with tf.Session() as sess:
    ...
    for epoch in range(20):
        for batch in range(393):
            batch_x1, batch_x2, batch_y = get_batch(fea1, fea2, labels, num_batch, batch, 393)
            sess.run(optim, feed_dict={input_x1: batch_x1, input_x2: batch_x2, input_y: batch_y})
        val_acc = sess.run(acc,feed_dict={input_x1: test_fea1, input_x2: test_fea2, input_y: test_labels})
        print "epoch    " + str(epoch) + ": acc   "+ str(val_acc)
        if val_acc > max_acc:
            max_acc = val_acc
            saver.save(sess, './checkpoint_dir/model20190113.model', global_step=epoch)
    sess.close()